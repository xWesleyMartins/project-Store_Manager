...
describe('testando camada Model', () => {
  test('Verifica se todos os produtos são mostrados caso não seja passado id', async () => {
    sinon.stub(connection, 'execute').resolves([mockAllProducts]);
    const result = await productsModel.findAll();
    expect(result).to.be.deep.equal(mockAllProducts);
  });
  test('Verifica se ao passar um "id" é retornado o valor correspondente ao id passado.', async () => {
    sinon.stub(connection, 'execute').resolves([mockAllProducts])
